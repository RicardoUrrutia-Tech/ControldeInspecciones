...
# =========================================================
# SSO / Acceso restringido
# =========================================================
CORP_DOMAIN = "@cabify.com"

def require_login_and_domain():
    if "auth" not in st.secrets:
        st.error("No se encontró configuración [auth] en Secrets.")
        st.stop()
...
# =========================================================
# <<< FIX >>> Parseo de fechas robusto
# =========================================================
def try_parse_date(series: pd.Series) -> pd.Series:
    # 1) Parseo normal
    dt = pd.to_datetime(series, errors="coerce", dayfirst=True)

    # 2) Intento como número Excel
    mask = dt.isna()
    if mask.any():
...
df["Días desde última inspección"] = (
    today - df["Última Fecha Inspección"]
).dt.days
df["Semáforo"] = df["Días desde última inspección"].apply(
    lambda x: traffic_light(x, 7, 30)
)

# =========================================================
# Gráfico (barras moradul)
# =========================================================
