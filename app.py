...
# ✅ FIX robusto contra FloatingPointError
def try_parse_date(series: pd.Series) -> pd.Series:
    # 1) Parseo normal
    dt = pd.to_datetime(series, errors="coerce", dayfirst=True)

    # 2) Para lo que falle: intentar como número Excel (días desde 1899-12-30)
    mask = dt.isna()
    if mask.any():
        numeric = pd.to_numeric(series[mask], errors="coerce")

        # saneos para evitar inf / valores absurdos
        numeric = numeric.replace([np.inf, -np.inf], np.nan)
        numeric = numeric.where(numeric.between(-10000, 200000))
...
def to_excel_bytes(df: pd.DataFrame) -> bytes:
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        fill_ok = PatternFill("solid", fgColor="DFDAF8")
        fill_alert = PatternFill("solid", fgColor="F5F1FC")
        fill_crit = PatternFill("solid", fgColor="F5F1FC")
...
# Gráfico distribución (más pequeño + barras moradul)
...
if vals.empty:
    ...
    with g_left:
        ...
        plt.hist(vals, bins=bins, color="#4A2B8D")  # ✅ moradul
        plt.xlabel("Días")
        plt.ylabel("Cantidad")
